...
' namespace API {
' --------------------------------------
Public Function ex_TryLookup( _
    ...
    sqlText = "SELECT TOP 2 [" & resultHeader & "] FROM " & tableRef & _
        ...
        "]), '', [" & keyHeader & "])))) = '" & _
        ex_EscapeSql(VBA.UCase$(ex_Helpers.private_Text_Normalize(keyValue))) & "'"
    ...
    If recordset.EOF Then
        ...
        GoTo CleanExit
    End If
    outValue = ex_ReadText(recordset, resultHeader)
    recordset.MoveNext
...
Public Function ex_EscapeSql(ByVal valueText As String) As String
    ex_EscapeSql = VBA.Replace$(valueText, "'", "''")
End Function

Public Function ex_ReadText( _
    ByVal recordset As Object, _
    ByVal fieldName As String _
) As String
    If VBA.IsNull(recordset.Fields(fieldName).Value) Then Exit Function
    ex_ReadText = ex_Helpers.private_Text_Normalize( _
