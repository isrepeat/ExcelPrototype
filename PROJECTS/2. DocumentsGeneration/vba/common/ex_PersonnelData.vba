...
Public Function ex_TryExecuteShpoCandidateSql( _
    ...
    Dim candidateValues() As Variant
    ...
    Do While Not recordset.EOF
        ReDim candidateValues(0 To candidateFieldNames.Count - 1)
        fieldIndex = 0
        For Each fieldName In candidateFieldNames
            candidateValues(fieldIndex) = ex_ExternalTables.ex_ReadText( _
                recordset, VBA.CStr(fieldName))
            fieldIndex = fieldIndex + 1
        Next fieldName
        outCandidates.Add candidateValues
        recordset.MoveNext
    Loop
    ex_TryExecuteShpoCandidateSql = True
...
CleanExit:
    ...
    Exit Function
EH:
    ex_Helpers.LogError "SHPO candidate query failed | Number=" & _
        VBA.CStr(VBA.Err.Number) & " | Description=" & VBA.Err.Description
    ex_Helpers.ex_ShowErrorMessage "Failed to execute SHPO candidate query: [" & _
