...
Перед обработкой datasets движок собирает позиции начала всех фактически
присутствующих `scope`. Конец текущей секции ограничивается ближайшим следующим
известным началом либо концом документа. Поэтому порядок секций может меняться,
а промежуточные опциональные секции — отсутствовать. Завершающий lookahead в
конкретном scope остаётся частью распознавания, но не является единственной
границей данных.
