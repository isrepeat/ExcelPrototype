...
Private Sub obj_IControl_Configure(ByVal controlNode As Object)
    ...
    If controlNode Is Nothing Then
        MsgBox "TableSingle: control node is not specified.", vbExclamation
        Exit Sub
    End If

    m_ControlName = Trim$(ex_XmlCore.m_NodeAttrText(controlNode, "name"))
    If Len(m_ControlName) = 0 Then m_ControlName = "tableSingle"

    m_ItemsSourceRaw = Trim$(CStr(ex_XmlCore.m_NodeAttrText(controlNode, "itemsSource")))
    If Len(m_ItemsSourceRaw) = 0 Then
        MsgBox "TableSingle: itemsSource is not specified for control '" & m_ControlName & "'.", vbExclamation
        Exit Sub
    End If

    m_LayoutSheet = Trim$(ex_XmlCore.m_NodeAttrText(controlNode, "__layoutSheet"))
    If Len(m_LayoutSheet) = 0 Then
        MsgBox "TableSingle: runtime layout sheet is missing for control '" & m_ControlName & "'.", vbExclamation
        Exit Sub
    End If
    ...
    If m_RowStart <= 0 Or m_ColStart <= 0 Then
        MsgBox "TableSingle: invalid row/column start for control '" & m_ControlName & "'.", vbExclamation
        Exit Sub
    End If
    If m_RowEnd < m_RowStart Then
        MsgBox "TableSingle: control '" & m_ControlName & "' has invalid spanRows range.", vbExclamation
        Exit Sub
    End If
    If m_ColEnd < m_ColStart Then
        MsgBox "TableSingle: control '" & m_ControlName & "' has invalid spanCells range.", vbExclamation
        Exit Sub
    End If
...
Private Sub obj_IControl_Render(ByVal wb As Workbook)
    ...
    If Not m_IsConfigured Then
        MsgBox "TableSingle: control '" & m_ControlName & "' is not configured.", vbExclamation
        Exit Sub
    End If

    If wb Is Nothing Then Set wb = ThisWorkbook
    If wb Is Nothing Then
        MsgBox "TableSingle: workbook is not specified for control '" & m_ControlName & "'.", vbExclamation
        Exit Sub
    End If

    Set ws = mp_GetWorksheetByName(wb, m_LayoutSheet)
    If ws Is Nothing Then
        MsgBox "TableSingle: sheet '" & m_LayoutSheet & "' was not found for control '" & m_ControlName & "'.", vbExclamation
        Exit Sub
    End If

    If m_TableItems Is Nothing Then
        MsgBox "TableSingle: itemsSource is not resolved for control '" & m_ControlName & "'.", vbExclamation
        Exit Sub
    End If

    If Not mp_TryBuildRenderBufferSingle(valueBlock, styleSegments) Then Exit Sub
...
Private Function mp_TryReadLayoutLongAttr( _
    ...
    If Len(rawText) = 0 Then
        If isRequired Then
            MsgBox "TableSingle: runtime layout attribute '" & attrName & "' is missing for control '" & m_ControlName & "'.", vbExclamation
            Exit Function
        End If
        outValue = 0
    ...
    If Not IsNumeric(rawText) Then
        MsgBox "TableSingle: runtime layout attribute '" & attrName & "' must be numeric for control '" & m_ControlName & "'.", vbExclamation
        Exit Function
    End If
...
Private Function mp_TryBuildRenderBufferSingle(ByRef outValueBlock As Variant, ByRef outStyleSegments As Collection) As Boolean
    ...
    If availableCols <= 0 Or maxRows <= 0 Then
        MsgBox "TableSingle: invalid render bounds for control '" & m_ControlName & "'.", vbExclamation
        Exit Function
    End If

    If Not mp_TryResolveSingleTableModel(tableModel) Then Exit Function
    If tableModel Is Nothing Then
        outValueBlock = Empty
        mp_TryBuildRenderBufferSingle = True
        Exit Function
    End If

    tableColumnCount = tableModel.ColumnCount
    If tableColumnCount <= 0 Then
        MsgBox "TableSingle: table item has no columns.", vbExclamation
        Exit Function
    End If
    If tableColumnCount > availableCols Then
        MsgBox "TableSingle: control '" & m_ControlName & "' requires " & CStr(tableColumnCount) & _
...
EH_INVALID_ROW:
    On Error GoTo 0
    MsgBox "TableSingle: unsupported row object in table rows. Expected obj_Row.", vbExclamation
End Function
...
Private Function mp_TryResolveStylePreset( _
    ...
    Select Case LCase$(Trim$(styleKind))
        ...
        Case Else
            MsgBox "TableSingle: unsupported style segment kind '" & styleKind & "'.", vbExclamation
            Exit Function
    End Select
...
Private Function mp_TryResolveSingleTableModel(ByRef outTable As obj_TableDynamic) As Boolean
    ...
    If m_TableItems.Count = 0 Then
        mp_TryResolveSingleTableModel = True
        Exit Function
    End If

    If Not IsObject(m_TableItems(1)) Then
        MsgBox "TableSingle: itemsSource entry must be obj_TableDynamic or obj_Table.", vbExclamation
        Exit Function
    End If

    Set tableItem = m_TableItems(1)
    If Not mp_TryResolveTableModel(tableItem, outTable) Then Exit Function
    mp_TryResolveSingleTableModel = True
End Function
...
Private Function mp_TryResolveTableModel(ByVal tableItem As Variant, ByRef outTable As obj_TableDynamic) As Boolean
    ...
    Select Case LCase$(TypeName(tableItem))
        ...
        Case Else
            MsgBox "TableSingle: unsupported table model type '" & TypeName(tableItem) & "'. Expected obj_TableDynamic or obj_Table.", vbExclamation
    End Select
End Function
...
Private Function mp_ConvertFixedTableToDynamic(ByVal fixedTable As obj_Table) As obj_TableDynamic
    ...
    If fixedTable Is Nothing Then
        MsgBox "TableSingle: fixed table model is not specified.", vbExclamation
        Exit Function
    End If
