...
' может закрывать только существующую открытую запись. Для смены статуса
' обязательны ожидаемый тип предыдущего события и его открытое состояние.
Public Function IsExportAllowed( _
    ...
    ' записи читается выше: WORD использует его для ТВО и данных
