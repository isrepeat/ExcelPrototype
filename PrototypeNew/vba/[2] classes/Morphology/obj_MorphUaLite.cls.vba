...
BEGIN
  MultiUse = -1  'True
END
Attribute VB_Name = "obj_MorphUaLite"
Option Explicit
...
' Унифицированная точка входа для кода, которому не важна конкретная
' реализация склонения. Старый публичный API m_* оставлен ниже: он полезен
' специализированным обработчикам ФИО и обеспечивает перенос без потери API.
Public Function TryInflect( _
    ...
    If caseName <> CASE_GENITIVE And _
       caseName <> CASE_ACCUSATIVE And _
       caseName <> CASE_DATIVE Then Exit Function

    outText = m_InflectPhraseToCase(sourceText, caseName)
    TryInflect = True
End Function
